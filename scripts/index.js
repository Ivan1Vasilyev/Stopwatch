...
const polymorphButtonStart = (event, buttonNotThis) => {
  ...
  switch (buttonThis.value) {
    case 'interval': addInterval()
    break
    case 'start': playTimer(buttonThis, buttonNotThis)
    break
    case 'reset': resetAll(buttonThis, buttonNotThis)
  }
}
...
const polymorphButtonPause = (event, buttonNotThis) => {
  const buttonThis = event.target
  switch (buttonThis.value) {
    case 'continue': playTimer(buttonNotThis, buttonThis)
    break
    case 'pause': pauseTimer(buttonNotThis, buttonThis)
  }
}
